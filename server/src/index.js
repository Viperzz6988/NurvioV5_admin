...
async function ensureSchema() {
  ...
  await run('CREATE INDEX IF NOT EXISTS idx_leaderboard_game_score ON leaderboard (game, score DESC)');

  // Add admin-related columns if missing (best-effort)
  ...
  try { await run("ALTER TABLE users ADD COLUMN failed_login_attempts INTEGER DEFAULT 0"); } catch (_) {}
  try { await run("ALTER TABLE users ADD COLUMN locked_until TEXT"); } catch (_) {}

  // Blackjack balance per user (nullable until used)
...
app.use((err, _req, res, _next) => {
  ...
  if (message.includes('SQLITE') || message.includes('db is locked') || message.includes('no such table')) {
    console.error(new Date().toISOString(), 'DB error:', message);
    return res.status(503).json({ error: 'db_unavailable' });
  }
  console.error(err);
