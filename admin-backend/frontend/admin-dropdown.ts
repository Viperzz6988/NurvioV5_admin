...
async function fetchUsers(query = ''): Promise<UserRow[]> {
...
function createItemEl(user: UserRow): HTMLElement {
...
export async function initUserDropdown(rootId = 'userDropdown') {
  const root = document.getElementById(rootId)!;
  const toggle = root.querySelector<HTMLButtonElement>('.user-dropdown-toggle')!;
  const menu = root.querySelector<HTMLDivElement>('.user-dropdown-menu')!;
  const hidden = root.querySelector<HTMLInputElement>('#targetUserId')!;
...
// Expose globally for simple init in HTML
(window as any).initUserDropdown = initUserDropdown;